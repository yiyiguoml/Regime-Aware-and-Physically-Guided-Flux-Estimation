...
def virtual_temperature(temperature_k, mixing_ratio_g_kg):
    ...
    Args:
        ...
        mixing_ratio_g_kg: The mixing ratio in units g kg-1.
...
def kinematic_sensible_heat_flux(sensible_heat_flux_W_m2, air_density_kg_m3):
    """
    Calculate kinematic sensible heat flux <w' theta'> from the raw sensible heat flux.

    Args:
        sensible_heat_flux_W_m2: Sensible heat flux in W m-2 units
        air_density_kg_m3: Density of air derived from near surface temperature and pressure
...
def kinematic_latent_heat_flux(latent_heat_flux_W_m2, air_density_kg_m3):
    latent_heat_of_vaporization_J_kg = 2264705.0
    g_per_kg = 1000.0
    return latent_heat_flux_W_m2 / (latent_heat_of_vaporization_J_kg * air_density_kg_m3) * g_per_kg
...
def bulk_richardson_number(potential_temperature_k, height,
    ...
    Args:
        ...
        mixing_ratio_g_kg: The mixing ratio at the same height as the potential temperature in units g kg-1.
...
def friction_velocity(tau, density):
    ...
    Args:
        tau: The surface stress in N m-2
...
def potential_temperature_from_height(temperature_k, z_atm):
    """
    Estimate potential temperature and pressure from temperature and height
    using dry adiabatic lapse rate approximation.
    ...
    Returns:
        potential_temperature_k [K]
        pressure_hpa [hPa]
    """
    Td = 0.0098  # K/m
    theta = temperature_k + Td * z_atm
    ...
    p0 = 1000.0  # hPa
    pressure_hpa = p0 / (theta / temperature_k) ** (1 / rocp)

    return theta, pressure_hpa